...
## 2026-09-04, ops session — Ring CC-12: a receipt for every llms.txt we claim

**USER-NEEDED (all carried, nothing new):**
- **CC-16 — methodology call.** `Disallow: /` plus a narrow `Allow:` carve-out is classified **blocked**; cnbc.com's 09-02 merge makes a case for **restricted**. Changing `verdict()` moves published percentages, so it waits for a session with the user.
- **SS-7 — ShortSupply digest #1**, user-gated, now two days overdue by its own schedule. Today's ShortSupply data strengthens the honest framing rather than weakening it (below).
- **Carried:** Canicrawl launch gate (drafts are ready; the changelog now has eleven days of history), the shortsupply.io/.co/.today domain pick, and the cron-lateness playbook amendment.

**No ops session ran on 2026-09-03**, so this entry covers two crawl days.

**Cron: green both products, both days.** Canicrawl's scheduled runs were created 2026-09-03 **11:13:51 UTC** and 2026-09-04 **11:17:25 UTC**, both `success`; ShortSupply's 11:43:28 and 11:45:09 UTC, both `success`. Against nominal 06:17/06:47 that is ~5h late, the eighth consecutive late-not-dropped day, and the drift is stable rather than growing. Both pulls landed clean: snapshots for 09-03 and 09-04, 1,000 domains each.

### Two days of changelog, both worth keeping

- **amplitude.com, Bytespider `restricted → blocked` (09-03).** A hand edit, and a specific one: they appended a named `User-agent: Bytespider` group carrying `Content-Signal: search=no, ai-input=no, ai-train=no` and `Disallow: /`. Someone picked one crawler out of the pack and shut the door on it while leaving the rest of the file alone — the opposite species from the boilerplate cohorts CC-15 counts, and the same shape as webmd.com's CONSFE ticket edit on 09-02.
- **tailscale.com published an llms.txt (09-04).** Verified as genuine and not a soft-404 or an anti-bot page: 422 bytes of real Markdown opening `# Tailscale`, with a `>` summary line and a `## Docs` section — exactly the format, archived at `data/llmstxt/tailscale.com.txt`. This is the first llms.txt appearance since the CC-13/CC-14 sniff hardening, so it is also the first evidence the fixed gate still lets true positives through rather than only suppressing false ones.
- **CC-15 cohort series, third point.** Cloudflare Managed Content **5 → 5** (gamespot, kick, nexusmods, patreon, snopes), robotstxt.com/ai copied list **1 → 1** (launchpad.net), Content Signals **23 → 22**. The single departure, isolated by name from the archive rather than inferred from the count, is **stackadapt.com**, and it is a nice inversion of the usual story: they deleted `Content-Signal: search=yes, ai-input=yes, ai-train=yes` — a *permissive* declaration — while touching no `Disallow` line, so their access policy did not move an inch and no bot-flip was emitted. The cohort can shrink through sites dropping a preference they had granted, not only through sites dropping a block.

### Ring CC-12 — every `llmstxt: true` now carries evidence

`crawl.js` archives an llms.txt body only below the 256KB cap, so seven sites (zendesk, salesforce, unity3d, sourceforge, datadoghq, selectel, mailchimp) were recorded as publishing an llms.txt with **nothing stored behind the claim**. That is the precise failure mode CC-11, CC-13 and CC-14 each corrected a version of: an assertion nobody — including a later crawl — can re-check. The crawler now records `llmsBytes` and a 16-char `llmsHash` (SHA-256 prefix) for every `true`, archived or not. The cap does not move, no new dependency, no methodology change, one polite pass per day untouched.

`/health/` — the page that already publishes the denominator behind every percentage — gains a matching section, **"Receipts for every llms.txt we report"**: full body archived / hash + byte count / no receipt yet, each counted from the data at build time, with the receiptless sites named and the oversized ones listed with their hash and size once the field exists.

**Verified, not assumed.** The receipt logic was **unit-tested end-to-end** by loading `crawlDomain` with a stubbed fetch layer (a copy in the scratchpad, network replaced by fixtures — nothing was crawled): **8/8 checks pass**, covering a 300KB oversized body (`llmstxt: true`, `llmsBytes` = 300,014, `llmsHash` equal to an independently computed SHA-256 prefix, and the body correctly **not** archived), a small body (still archived, and now also carries a receipt), a 404 (no receipt fields invented), and CC-13's regression case — an `<a id="a-link">` anti-bot page is still rejected and produces no receipt. Build clean at **1,064 pages**; the section was read back out of `dist/health/index.html`, where the ledger reads 102 + 0 + 7 and **sums exactly to the 109 sites the site publishes as having an llms.txt**; all **7 member links resolve** to real `dist/site/<domain>/index.html` pages. `rebuild-changelog.js` dry run: **143 → 143, 0 dropped, 0 gained** — no regression on CC-11/CC-13/CC-14. Served `dist/`: HTTP **200** on `/health/`, `/changelog/`, `/stats/` and `/site/tailscale.com/`, with the new section present on the served page.
...
## 2026-09-06, ops session — Ring CC-18: the day nobody answered should not erase the day somebody published

**USER-NEEDED (all carried, nothing new):**
- **CC-16 — methodology call.** `Disallow: /` plus a narrow `Allow:` carve-out is still classified **blocked**; cnbc.com's 09-02 merge argues for **restricted**. Changing `verdict()` moves published percentages, so it waits for a session with the user.
- **SS-7 — ShortSupply digest #1**, user-gated, four days past its own due date. Today's ShortSupply data adds nothing new to the drafted framing.
- **Carried:** the Canicrawl launch gate (drafts ready; thirteen days of changelog history), the shortsupply.io/.co/.today domain pick, and the cron-lateness playbook amendment.

**Cron: green both products.** Canicrawl's scheduled run was created 2026-09-06 **10:55:43 UTC**, `success`; ShortSupply's **11:21:21 UTC**, `success`. Against nominal 06:17/06:47 that is ~4h39m and ~4h34m late — the tenth consecutive late-but-not-dropped day, and about 20 minutes later than yesterday, so the drift is wobbling rather than trending. Both pulls landed clean: 1,000 domains, 244 drugs.

### Today's diffs: eight changed robots.txt files, zero changelog entries — and this time one of them should have been reported yesterday

The pull carried changed robots.txt for blackberry.com, etsy.com, pinimg.com, pinterest.com, usatoday.com and wordpress.com, plus a changed llms.txt for porkbun.com, and the changelog stayed at 143. reg.ru is the honest silence: its robots.txt was a WAF HTML page again today after one clean day, so CC-11's `was.fetch === now.fetch` guard suppressed the round trip — our access changed, not its policy.

**time.com is the dishonest one, and it is yesterday's.** Its llms.txt series reads: definitively absent 08-25 → 08-29, then five straight days where our probe got no answer at all, then on **09-05 a real 733-byte file** (`# llms.txt - TIME's Policy for Large Language Model Access`, `disallow: *` with named exceptions), archived in this repo. Because the crawler only ever compares two consecutive days, and 09-04 was an unknown, the publication was suppressed — correctly, on the evidence available that morning. And then it was gone: today is unknown again, and any day after would be true→true. A national news magazine writing its first AI-access policy is exactly the event this index exists to catch, and the record would have shown nothing, forever.

### Ring CC-18 — compare against the last day that answered, not blindly against yesterday

The whole fix is one baseline selection. `computeDiffs(prev, next, history)` now takes the snapshots older than `prev`, oldest first, and **only when yesterday gave no definitive llms answer** walks backwards to the most recent day that did, stopping at **14 days** — beyond that a reading is no longer evidence about today. Everything the last four rings installed still applies *to the carried day*: it must pass CC-11's `fetch` guard against today, and its stored `true` is still re-tested against its archived receipt (CC-13/CC-14). `crawl.js` gains a small `loadLlmsHistory()` that parses each prior snapshot once and keeps only `fetch`/`llmstxt`/`llmsFetch`, so the cron reads a few MB instead of forty. No crawler behaviour changed, no dependency added, no published percentage moves.

Deliberately not loosened: the fetch guard. An anti-bot page that returns 200 makes a false "definitive no" cheap to manufacture, and the carry-forward would happily propagate one.

**Verified, not assumed.**
- **11/11 unit tests** against synthetic snapshots, no crawling: carries over one unanswered day; stays silent with no history; emits **no duplicate** the day after a carried flip; silent when the carried baseline agrees with today; **fetch guard holds on the carried day**; drops a baseline 15 days old and accepts one at exactly 14; walks past a run of three unanswered days; silent when *today* is the unknown; plain two-day flips unchanged; `unreachable` today never diffs.
- `loadLlmsHistory()` run against the real snapshot directory: 11 dates for a `prevDate` of 09-05, **excludes prev and everything later**, bounded at ≤14, and the compact rows carry exactly the three fields.
- Full **13-snapshot rebuild: 143 → 144, dropped 0, gained 1** — the gain is time.com on 09-05 and nothing else. shein.com, whose probe flaps on alternating days, gained **no** phantom flips, and its genuine 08-27 first sighting is preserved.
- The **production path** was checked separately from the rebuild script: feeding the 09-04 and 09-05 snapshots through `computeDiffs` with `crawl.js`'s own disk loader reproduces the time.com entry exactly, and 09-05 → 09-06 emits **0** diffs, matching what the cron published this morning.
- Build clean at **1,064 pages**. `/changelog/` renders "time.com published an llms.txt" under 2026-09-05; rss.xml carries it; **youku 0/0** on both, so no CC-13/CC-14 regression. time.com's site page correctly shows CC-17's *no answer today* plus the archived-copy link — the receipt argues for a file the index cannot see today. HTTP **200** on `/`, `/changelog/`, `/health/`, `/stats/`, `/site/time.com/`, `/changelog/rss.xml`.

The changelog is derived data, so this was fixed by regenerating it from the untouched append-only snapshots — the same move as CC-13, CC-14 and SS-6. No snapshot was edited; no crawl was run from this machine.

**Next:** CC-10 (Tranco weekly refresh) is Monday work by its own description and is now seven days overdue — 09-07 is a Monday, so it is next session's ring. Digest #3's lead keeps growing: three of its beats are now *our own* corrections, and time.com is the cleanest example yet of a real policy event that a naive daily differ would have thrown away.

## 2026-09-07, ops session — Ring CC-10: the panel had started to drift, and nothing was watching

**USER-NEEDED (one new, the rest carried):**
- **New — the exclusion list catches infrastructure by name, not by nature.** Six of today's twenty newcomers are plainly not websites: `usercontent.goog`, `bytefcdn.com`, `dnse0.com`, `dnse2.com`, `statsigapi.net`, `immedia-semi.com`. They pass because `INFRA_SUFFIXES` is a hand-written list of known hosts, and these are not on it. The founding decision, published on `/health/` and `/about/`, is deliberate — *"we keep them listed and counted rather than curating the panel by hand"* — so I added them rather than quietly filtering, which is why this is a note and not a fix. But there is a trap in extending the list later: `expand-panel.js` has a hygiene pass that **prunes** existing `top1k` members the current exclusion list would now reject, so growing `INFRA_SUFFIXES` would silently delete tracked domains and their history. If we ever tighten the filter, that pruning must be removed in the same change.
- **Carried — CC-16**: the `Disallow: /` + narrow `Allow:` case (cnbc.com). Still unanswered; changing `verdict()` moves published percentages.
- **Carried — launch gate** (Canicrawl first), the ShortSupply domain pick, and SS-7.

**Cron: green both products, late again.** Canicrawl's scheduled run created **12:32:05 UTC**, `success`; ShortSupply's **13:01:23 UTC**, `success`. That is ~6h15m past the nominal 06:17/06:47 — the latest yet, and the **eleventh** consecutive day of late-but-delivered. Late is not dropped: both snapshots exist and both deploys landed.

**A quiet data day.** Canicrawl's changelog did not move: **144 entries**, last dated 09-05, nothing on 09-06 or 09-07. Seven raw `robots.txt` files changed and none of them meant anything — pinterest.com and pinimg.com reshuffled the *order* of eighteen sitemap lines, blackberry/etsy/kayak/wordpress moved a byte each. Worth recording that the differ stayed silent through that: a sitemap reordering is exactly the kind of churn a naive text-diff would have published as news. One genuine coverage gain: `xhamster46.desi` returned a readable robots.txt for the first time and is now archived. ShortSupply was flat too — 55 entries, last dated 09-05; **244 drugs, 70 in shortage**, both unchanged, so no drug entered or left the list and the graveyard still holds three.

**The ring: CC-10, the weekly Tranco refresh.** The panel was filled once, on 2026-08-25, by walking Tranco list `38LNL` in rank order and taking everything that survived the exclusion filters, stopping at 1,000. Nothing has re-examined it in the thirteen days since, and rankings move. Against today's list (`74V6X`, built 2026-09-06) the panel had drifted by **forty domains in both directions**: twenty sites had risen into the band we cover and we were not tracking them — including `usps.com` (rank 856), `ryanair.com` (913), `xerox.com` (976), `hostinger.com` (327), `epam.com` (931) — and twenty we do track had fallen out of it, `workbox.dk` all the way to rank 4141, `nominetdns.uk` and `nic.direct` out of the top 6,000 entirely. Two weeks in, 2% of the panel per week is no longer "the top sites" in either direction.

`scripts/refresh-panel.js` re-runs the founding algorithm against the current list and reports the difference. Two properties make it safe to run unattended:

- **Additions only.** A domain that falls out of the band is *reported* and *kept*. Its snapshots are append-only facts and its site page is a live URL; deleting it would break both. So the panel grows and never churns — 1,000 → **1,020** today.
- **Bounded.** At most 25 additions per run, best-ranked first, so a bad list day cannot reshape the panel while nobody is watching. Today's twenty fit under the cap with none deferred.

`data/panel-history.json` is a new append-only ledger recording each refresh: the list id, the panel size before and after, every domain added with its rank, anything deferred, and every departure kept. The panel's composition is now reproducible from the repo the same way its statistics already were — the same instinct as CC-12's llms.txt receipts, applied to the denominator itself.

Nothing was crawled. `crawl.js` reads `domains.json`, so the twenty new domains are picked up by tomorrow's 06:17 pass as part of its one polite daily crawl; `build.js` reads its domain list from `latest.json`, so today's site is byte-for-byte unaffected and tomorrow's grows by twenty rows.

**A build-side bug the ring surfaced.** Two published sentences had "1,000 domains" typed into them — the digest page's meta description and the palewire prior-art paragraph on `/about/` — which would have gone stale tomorrow. Making them read from the data caught that the digest one is a plain double-quoted string, not a template literal, so the first attempt shipped the literal text `${commas(DOMAINS.length)}` into the meta description of a live page. Found by grepping the built HTML rather than the source, which is the only reason it was caught at all. Fixed to a template literal; the bare-count style now matches the four places that already interpolated it.

**Verified, not assumed.**
- `panelDelta`/`reconstructFill` unit-tested on synthetic panels: **13/13** — a stable list produces zero delta in both directions, a riser is detected with its rank and displaces exactly the worst-ranked incumbent, a departure outside the fetched list gets a `null` rank rather than a guess, curated domains are never reported as departures and never re-added as `top1k`, infra and adult domains are excluded and the freed slot goes to the next eligible site, and case/whitespace in the list produce no phantom newcomers.
- The mutation itself re-checked against a pre-run copy of `domains.json`: **0 domains removed**, **0 categories changed on existing members**, 20 added and all 20 categorised `top1k`, and the ledger's `added` list matches the actual file delta exactly. All 20 departures are still in the panel.
- Tomorrow's differ tested by replaying `computeDiffs` over a synthesised next-day snapshot: **4/4** — exactly **20 `added` entries**, exactly the 20 new members, **zero** bot-flip/llms/wildcard noise manufactured by the panel change, and no departure reported as removed.
- `expand-panel.js` now exports its filters behind a main guard; verified by importing it and confirming `domains.json` was **byte-identical afterwards** — the one-shot expansion did not run.
- **1,064 pages** built. Interpolation confirmed *in `dist/`*: "census of 1000 top sites", "all verticals, 1000 domains", "The panel is 1000 domains" — and `grep -rl 'DOMAINS.length' dist/` returns **nothing**, so no template syntax leaked. All five spot-checked newcomers correctly have **no page yet**; all five spot-checked departures **keep** theirs.
- `rebuild-changelog.js` dry run **144 → 144, 0 dropped, 0 gained**.
- HTTP **200** on `/`, `/about/`, `/health/`, `/stats/`, `/changelog/`, `/digest/`, `/site/otto.de/`, `/site/umich.edu/`, `/changelog/rss.xml`. ShortSupply: **252 pages**, HTTP 200 on `/`, `/changelog/`, `/graveyard/`, `/changelog/rss-cardiovascular.xml`, all three graveyard members present by name, disclaimer on the homepage.

**Next:** the refresh is a Monday job — next run 2026-09-14, and the interesting number will be whether ~20/week is the steady rate or an artifact of a thirteen-day gap. Tomorrow's changelog should carry twenty `added` entries and the panel prose should read 1,020. CC-16 and SS-7 still want the user.

## 2026-09-08 — CC-19: the ledger CC-10 wrote is now a page anyone can read

**Crons:** both green, both late. Canicrawl's scheduled run landed 11:18 UTC against a 06:17 cron and ShortSupply's 11:44 UTC against 06:47 — five hours of GitHub queueing delay, on top of yesterday's six (12:32 / 13:01). Both completed successfully and committed their snapshots, so this is upstream scheduling jitter and not ours to fix; noting it because two consecutive days well outside the "up to an hour" norm is worth a trend line. Snapshot `2026-09-08.json` present in both repos.

**Yesterday's forecast, checked.** CC-10 predicted today's changelog would carry exactly twenty `added` entries and that the published prose would read 1,020. Both hold: 144 → **180 entries**, of which **20 are `added`** and they are exactly the twenty domains in `panel-history.json` — no bot-flip or llms noise manufactured by the panel change, which is what the replayed differ said would happen. `domains.json` holds 1,020, `/about/` reads "The panel is 1020 domains", and the twenty newcomers have site pages for the first time (build 1,064 → **1,084 pages**).

**Notable flips banked for the digest.** Sixteen real policy changes today, in three stories. (1) A **Condé Nast cohort**: newyorker.com, wired.com, epicurious.com and bonappetit.com all moved YouBot restricted → blocked on the same morning — four properties, one publisher, one decision, exactly the "blocklists move in cohorts" thread CC-15 opened and CC-20 is meant to lead on. (2) **intel.com opened up**, moving GPTBot, ChatGPT-User, ClaudeBot, Google-Extended, PerplexityBot and anthropic-ai from restricted to allowed. (3) **lg.com went the other way** on six of the same names, allowed → restricted. Two large manufacturers moving in opposite directions on the same day is a better pairing than either alone.

**Ring executed: CC-19 — publish the panel ledger.** CC-10 made the denominator reproducible *in the repo*; a reproducibility claim nobody can see is the same failure mode this index has corrected repeatedly on the data side. `/health/` now has **"How the panel changes"**, generated from `data/panel-history.json` at build time: the weekly-refresh rule stated (additions only, capped at 25/run, so one bad list day cannot reshape the census unattended), a table of every refresh — date, Tranco list id, panel before → after, added / kept / deferred — and a per-refresh disclosure naming all forty members of the 09-07 run with their ranks. The paragraph that matters is the second one: **a domain that falls out of the ranking stays in the index**, because dropping it would delete history — its snapshots are append-only facts about what that robots.txt said on those mornings, and `/site/<domain>/` is a live URL somebody may have linked. That sits directly above the existing no-DNS paragraph, which makes the same argument about a different kind of awkward member. Read-only: no methodology change, no new data, nothing crawled.

**Verified, not assumed.**
- Built **1,084 pages**. The section was read back out of `dist/health/index.html`, not out of the source: counts there are **1 refresh, 20 added, 20 kept, 1,000 → 1,020**, matching `panel-history.json` field for field, and `domains.json` independently holds 1,020.
- **40/40 site links inside the section resolve** to real generated pages under `dist/site/`, and the rendered order equals the ledger order exactly — so the page cannot silently drop or reorder a member.
- Both `null`-rank departures (`nominetdns.uk`, `nic.direct`) render **"outside the list"** rather than a fabricated rank; `/site/nominetdns.uk/` returns 200, confirming the keep-don't-delete claim is true and not just written.
- One defect caught and fixed during the build: the first draft called a `commas()` helper that exists in `refresh-panel.js` but not in `build.js` — the build threw rather than shipping, and it now uses a local `num()`.
- `rebuild-changelog.js` dry run **180 → 180, 0 dropped, 0 gained** (15 snapshots, 5 llms flaps repaired, 1 receipt rejected — all pre-existing).
- HTTP **200** on `/`, `/health/`, `/stats/`, `/changelog/`, `/site/xerox.com/`, `/site/usps.com/`, `/site/otto.de/`, `/site/nominetdns.uk/`, `/changelog/rss.xml`.

**Next:** CC-20 (digest #3) is the natural follow-on and now has a second cohort story — the Condé Nast YouBot move alongside CC-15's Cloudflare/Content-Signal series and the intel/lg split. Re-count all three cohorts at write time for a third data point. CC-16 and SS-7 still need the user.

## 2026-09-09 — CC-20: digest #3, and the count that moved because we did

**Crons:** both green. Canicrawl's scheduled run created 11:22 UTC against a 06:17 cron, ShortSupply's 11:52 UTC against 06:47 — five hours of GitHub queueing delay for the third day running (09-07: six hours, 09-08: five). Both completed, both committed `data/snapshots/2026-09-09.json`. Three consecutive days of five-to-six-hour lag is now a pattern rather than jitter, but it is upstream scheduling and costs us nothing: the snapshot still lands on the correct UTC date and the deploy still ships. Worth a line each session until it either normalises or starts eating into the next day.

**Notable flips.** A quiet day after yesterday's sixteen: **183 changelog entries**, three new. `t-online.de` moved Google-Extended blocked → restricted, `welt.de` moved Amazonbot restricted → blocked, and **intel.com published a 12,892-byte `llms.txt`** — one day after moving six AI crawlers from restricted to explicitly allowed. Intel now blocks **0 of our 32** and hands agents a map. That pairing (open the door on Monday, put up a signpost on Tuesday) went straight into the digest.

**Ring executed: CC-20 — "State of the Agent Web #3", live at `/digest/3/`.** Title: *Most blocklists are not written, they are moved.* CC-15 banked the lead; ten more days of data turned it into a thesis with three varieties and a counter-example.

- **The CDN toggle.** Cloudflare Managed Content holds at **5 → 5 → 5** with the same five members (gamespot, kick, nexusmods, patreon, snopes). The reason to watch a flat number: on 09-01 roblox.com deleted its managed block and eight crawlers went blocked → allowed in one move.
- **The copied list.** `robotstxt.com/ai` is down to **one** member (launchpad.net) and has held there a week; it lost the other when semafor.com deleted a 96-line copied roster on 09-02 and un-blocked **31 of 32** in a single commit. Semafor's one remaining block is GPTBot — confirmed from `latest.json`, not from memory.
- **The fleet edit.** New material this session: on 09-08 newyorker.com, wired.com, epicurious.com and bonappetit.com all moved YouBot restricted → blocked, and the four robots.txt files took an **identical 33-added / 24-removed diff** on the same morning. All four are Condé Nast; all four now block 20 of 32. The detail that makes it a story is arstechnica.com — also Condé Nast — which has named YouBot in a `Disallow: /` group on **every readable day since the census began** (15 of 16 days; 09-01 its file was unreadable to us). The fleet did not decide, it converged.
- **The counter-example.** webmd.com's robots.txt header carries its own ticket trail, `CONSFE-362` (3/4/2026) → `CONSFE-458` (8/31/2026), and on 09-01 exactly one thing changed: ChatGPT-User stopped being blocked while GPTBot, ClaudeBot and CCBot — the only three of our 32 it blocks — stayed. Somebody drew a line between a user's request and a training run and filed a ticket for it. That is rarer in this data than any cohort deletion.

**The honesty section is the one I would defend hardest.** Our live cohort counter reads **24** Content Signals sites today. The like-for-like number is **22**. The gap is entirely our own denominator: CC-10's 09-07 Tranco refresh added 20 domains, and two of them (bitdefender.net, hostinger.com) were already carrying the header when we first read them on 09-08. Recomputed from the committed archive at each day's own commit, the real series is 23 (09-02) → **22** (09-04, after stackadapt.com dropped the header) → 22 today: one departure, **zero adoptions in eight days**. The issue says so in its own section. This is the same failure mode as issue #2's Amazon regional storefronts — a count can move because the web moved or because our lens did, and only one of those is news.

**Verified, not assumed.**
- Built **1,084 → 1,085 pages**; `/digest/3/` exists, `/digest/` lists issue #3, `sitemap.xml` contains `digest/3/`, and no template syntax leaked into the output.
- **32/32 internal links in the issue resolve** to real generated pages under `dist/`.
- Every load-bearing claim was read back out of `dist/digest/3/index.html` rather than out of the source: "5 today", "23 → 22", "1,020", "20 of the 32", "33 lines added, 24 removed", "CONSFE-458", "12,892-byte", "31.6% of the 649", "112 sites publish", "624 domains", "un-blocked 31 of the 32".
- Historical cohort sizes were **recomputed from git at each day's snapshot commit** (`git grep -lIiE` over `data/robots` at that rev), not remembered from CC-15's journal entry; that is what caught the 24-vs-22 discrepancy in the first place. Membership diff 09-02 → today: left `stackadapt.com`, joined `bitdefender.net` + `hostinger.com`.
- Headline figures taken from build.js's own output (`31.6% of 649` readable; 112 published / 512 none found / 396 no answer, summing to 1,020, with 11% stated as a floor over the 624 that answered) rather than recomputed with a private definition.
- One defect caught before commit: the intro rendered "1020 top websites" without a thousands separator, unlike issues #1 and #2. Fixed and re-verified in `dist/`.
- `rebuild-changelog.js` dry run **183 → 183, 0 dropped, 0 gained** (16 snapshots, 7 llms flaps repaired, 1 receipt rejected — all pre-existing).
- HTTP **200** on `/`, `/digest/`, `/digest/3/`, `/digest/2/`, `/stats/`, `/health/`, `/changelog/`, `/site/intel.com/`, `/site/newyorker.com/`, `/site/stackadapt.com/`, `/changelog/rss.xml`. Data-only change: `data/digests.json`, +7 lines, nothing crawled.

**Next:** CC-21 (compare view) is the only ungated ring left in the queue — after that the queue needs new rings appended from the pillars. Watch whether the Condé Nast fleet edit propagates to more of its titles, and whether Content Signals adoption stays at zero now that we can tell adoption from panel growth. CC-16 (the `Disallow: /` + narrow `Allow:` classification question) and SS-7 (ShortSupply digest #1) both still need the user.

## 2026-09-10 — CC-21: any two sites, side by side, as a link

**USER-NEEDED (standing, unchanged):** CC-16 (whether `Disallow: /` plus a narrow `Allow:` carve-out should read *restricted* rather than *blocked* — moves published percentages, so it is your call) and SS-7 (ShortSupply digest #1) are the only gated rings in the queue. Nothing new escalated today.

**Crons:** both green. Canicrawl's scheduled run was created **11:18 UTC** against a 06:17 cron, ShortSupply's **11:49** against 06:47 — the **fourth** consecutive day of five-hour GitHub queueing delay across both repos. Both completed and committed `data/snapshots/2026-09-10.json`; the snapshot still lands on the right UTC date. Upstream scheduling, still costing nothing, still worth one line a day.

**Notable flips.** Two changelog entries (183 → 185), one story. **hostinger.com moved CCBot and Bytespider restricted → blocked**, and the robots.txt diff shows how: it rewrote its own Content Signals from `ai-train=yes` to **`ai-train=no`** in both groups that carry them, and appended explicit `Disallow: /` groups for the two bots under the comment *"Crawlers that only collect content for model training"* — while leaving `ai-input=yes` and its long allow-list of assistant and search agents (ChatGPT-User, DuckAssistBot, YouBot, MistralAI-User…) untouched. A hand edit that draws exactly the line webmd.com drew in digest #3: answer a user's request, refuse a training run. It is also the first *value* change inside the Content Signals cohort we have seen — hostinger joined the panel with the header already present on 09-08 (digest #3's honesty section), so the cohort count does not move, but what the header says did. Digest #4 material alongside the Condé Nast watch. (Hostinger also carries non-standard `Agentmap:` and `EntityMap:` directives pointing at `/.well-known/ai-catalog.json`; it is the only file in the archive that does — a curiosity, not yet a cohort.)

**One headline site went dark to us:** nytimes.com's robots.txt came back as an HTML page (`html-response`) for the first time — `ok` every day 09-01 → 09-09. CC-11's guard did its job: no flip was emitted, the site reads *unknown* rather than "unblocked". Checked this wasn't a UA-reputation spike per the playbook: day-over-day outcome churn is 8 domains in both directions (nasa.gov and gnu.org recovered, gosuslugi.ru and stbid.ru went unreachable), html-response 42 → 44. One site, not a pattern. If it persists, the `/api/` page's worked example (nytimes.com.json) will keep showing an unreadable record — worth swapping the example then, not today.

**Ring executed: CC-21 — `/compare/`.** Two tracked sites side by side across all 32 bots, pair in the query string (`/compare/?a=newyorker.com&b=arstechnica.com`), so a comparison is a shareable link. Smallest shape that works: a plain GET form (the URL exists even with JS off) plus an inline script that fetches the two per-site JSON files we already publish — no new data, no crawler change, no dependency. Rows: robots.txt outcome, default `*` policy, llms.txt in CC-17's three states (published / none found / no answer today), then every bot with the site pages' own chips and inherited asterisk; disagreeing rows are marked, a toggle hides the rest, and one sentence summarises ("newyorker.com blocks 20 of the 32…; arstechnica.com blocks 23. They disagree on 3 of 32 bots"). An unreadable site compares as unknown on every row and the page says so in words instead of implying the other site is stricter. Example pairs are filtered at build time to sites readable that day — nytimes.com vs theguardian.com dropped itself today, which is the filter working. Every site page gains a "Compare" link prefilled with itself; `sitemap.xml` and `llms.txt` list the page.

**Verified, not assumed.**
- Build **1,085 → 1,086 pages**; `dist/compare/index.html` 39KB (the 1,020-option datalist is most of it); **0** `${` leaked into it.
- Headless harness running the page's **own inline script** (extracted from `dist/`) against a small DOM stub, with `fetch` served from `dist/`: **29/29**. All 32 bot cells for newyorker.com vs arstechnica.com re-derived from `latest.json` — **0 mismatches** across status, inherited marker and diff class; summary counts 20 / 23 / 3 match; `HTTPS://www.Intel.com/about` normalises to intel.com; an unknown domain, an `<img onerror>` payload and `../../etc` each fetch **nothing**; one-site and no-param states behave; nytimes.com renders unknown ×32 with "HTML instead of text"; the three llms states render from real domains (foxnews.com / bbc.com / apnews.com).
- **1,020/1,020** site pages carry the compare link.
- HTTP **200** on /, /compare/, /compare/?a=intel.com&b=lg.com, /site/intel.com/, /site/shein.com/, /data/sites/intel.com.json, /stats/, /health/, /changelog/, /digest/3/, /sitemap.xml, /llms.txt — through an in-process static server, because unattended sessions cannot start the preview server. **Not yet seen in a real browser**; the next attended session should eyeball it once (layout at mobile width especially).
- One defect caught before commit: the "only rows that differ" state wasn't re-applied when a browser restores the checkbox on back/forward. Fixed, rebuilt, 29/29 again.
- No snapshot edited, nothing crawled; `crawl.js` and the differ untouched, so no changelog rebuild was needed.
...
## 2026-09-20 — Ops: SS-13 verified (no fix needed), SS-14 queued; zero flips on both products

**USER-NEEDED (standing, unchanged):** CC-24 (the highwebmedia.com filter gap, which still holds the weekly Tranco refresh), CC-16 (Allow carve-outs under `Disallow: /`), SS-7 (ShortSupply digest #1). Nothing new escalated today.

**Crons:** both green. Canicrawl's 09-20 scheduled run was created **11:23 UTC** against a 06:17 cron and ShortSupply's **11:49 UTC** against 06:47 — ~5 h of GitHub queueing in both cases, the same shape as 09-18 and 09-19. Both committed their `2026-09-20` snapshots, and both repos pulled clean and fast-forward.

**Notable diffs — the quietest day since tracking began.** Canicrawl's changelog is flat at **293** entries with **0** dated 09-20, and ShortSupply's at **102** with 0. What did move on Canicrawl is worth recording because it moved *without* moving a verdict: eight archived bodies changed — `data/robots/{dell.com, etsy.com, foxnews.com, pinimg.com, pinterest.com, wordpress.com, xerox.com}.txt` and `data/llmstxt/asus.com.txt` (130 lines) — and grepping those diffs for user-agent lines and AI-bot tokens (GPTBot, ClaudeBot, CCBot, PerplexityBot, *-Extended, Bytespider, anthropic) returns **nothing**. The churn is entirely in non-AI sections, which is exactly what the hash-plus-verdict snapshot design is supposed to absorb silently. `data/discovery.json` also changed by a single line — the date — so the new-token scan found no new AI-ish candidates across 610 files / 2,078 distinct tokens. Yesterday's wiley.com replatforming did not spread and did not revert.

**Ring executed: SS-13** (full entry in the sibling repo). It was a verification ring and its premise turned out to be wrong, so **no code changed in either repo**. The ring assumed /graveyard/ had been empty since SS-2 and that the two 09-19 departures would be its first rows; in fact the changelog holds **10** `removed` entries back to 2026-08-26 and the page has been rendering them all along. Verified: a 250-page build on the 09-20 snapshot, 10 graveyard rows newest-first including both 09-19 departures with dates and last-seen status, the empty state absent, in-process HTTP 200 on nine paths, and the disclaimer on 250/250 pages. Canicrawl itself rebuilt clean at **1,087 pages** on the 09-20 snapshot. No crawl was run; no snapshot was touched.
- **Found while verifying:** the ring's second clause — that departed drugs' pages survive as live URLs — is false. ShortSupply builds drug pages and per-drug JSON only for drugs on today's list, so a URL disappears the day its drug leaves the FDA list. Live today: `/shortsupply/drug/methotrexate-injection/` **404**, `/shortsupply/drug/gemcitabine-hydrochloride-injection/` **404**, `/shortsupply/data/drugs/methotrexate-injection.json` **404**, against **200** for `/shortsupply/graveyard/`. Ten drugs are in that state. Nothing internal links to them, so it is indexed/external link rot rather than a broken site, but it reads badly beside the archive promise. Building tombstone pages is more than "fix the graveyard query", so it is queued rather than done.
...
## 2026-09-22 — Ops: CC-25 shipped (digest #5, "Blocklists that left with the plumbing"); zero flips on both products

**USER-NEEDED (standing, unchanged):** CC-24 (highwebmedia.com filter gap, still holding the weekly Tranco refresh; this Monday's refresh therefore also skipped), CC-16 (Allow carve-outs under `Disallow: /`), SS-7 (ShortSupply digest #1). Nothing new escalated.

**Crons:** both green. Canicrawl's 09-22 scheduled run was created **11:37 UTC** (cron 06:17) and ShortSupply's **12:03 UTC** (cron 06:47). That is ~5.3 h of GitHub queueing, a bit better than 09-21's ~6.5 h. Both committed `2026-09-22` snapshots and both repos pulled fast-forward. SS-14's archive URLs are **200 live** (methotrexate-injection, gemcitabine-hydrochloride-injection, /graveyard/).

**Notable diffs (09-22):** changelog flat at **293** (0 dated 09-22), ShortSupply flat at **102**. Raw body changes, none moving a verdict: wiley.com gained a second Webflow sitemap line plus an `Allow: /` that sits above any User-agent line (in no group, so it applies to nobody); coursera.org added an AdsBot-Google allow group; time.com a BomboraBot allow; trustpilot one `/users/connect?intent` disallow repeated in every group; quora moved its `_POST` rules up within each group. **gamespot.com has been fetch-blocked (401/403) every day since 09-16.** It still counts in the Cloudflare Managed cohort from its archived file, so I queued **CC-26** to label stale cohort members.

**Ring executed: CC-25.** Issue #5 appended to `data/digests.json` (data only). Every number was re-derived from the archive at the relevant commit: patreon and kick deleted the identical 61-line Cloudflare managed section on 09-16; wiley's 33 flips come from a replatform, not a policy change; weather.com's CW-9994 comment says training crawlers "remain blocked" while the same edit unblocked GPTBot, Google-Extended, Applebot-Extended and six others; onet.pl added 14 groups including Ai2Bot-Dolma; azure.com's llms.txt is gone. Cohorts: CF Managed 5 → 3, robotstxt.com/ai 1, Content Signals 25 → 24. Headline unchanged at 31.9% of 646 (206), though the membership underneath moved (−kick −gamespot, +onet.pl +stbid.ru). **Verified:** 1,088 pages; JSON diff +7 lines; 46/46 internal links resolve; 23/23 claims read back from `dist/digest/5/`; sitemap and index list #5; `rebuild-changelog.js` dry run 293 → 293, 0/0; HTTP 200 on 16 URLs (in-process server). Not yet checked live; that happens after this push deploys.
